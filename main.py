...
# ✅ 50+ FOREX PAIRS — Full market scan via yfinance
SCAN_PAIRS = [
    # Majors
    "EURUSD=X", "GBPUSD=X", "USDJPY=X", "USDCHF=X",
    "USDCAD=X", "AUDUSD=X", "NZDUSD=X",
    # Euro crosses
    "EURGBP=X", "EURJPY=X", "EURAUD=X", "EURCAD=X",
    "EURCHF=X", "EURNZD=X",
    # GBP crosses
...
def pair_display(ticker: str) -> str:
    """Convert yfinance ticker to readable pair e.g EURUSD=X -> EUR/USD"""
    raw = ticker.replace("=X", "")
    if raw in ("XAUUSD", "XAGUSD"):
        return raw[:3] + "/" + raw[3:]
    return raw[:3] + "/" + raw[3:]
            ...
            try:
                ...
                if hit:
                    send_message(alert["chat_id"],
                        ...
                        f"*{alert['pair']}* has reached *{current_price:.5f}*\n"
...
def set_bot_commands():
    commands = [
        ...
        {"command": "signal",       "description": "Smart scan full forex market"},
        ...
        {"command": "reset",        "description": "Reset account and lot size"},
...
def get_candles(ticker: str, interval: str, size: int = 50):
    """Fetch OHLCV candles from yfinance. No API key needed."""
    try:
        yf_interval = YF_INTERVAL[interval]
        yf_period   = YF_PERIOD[interval]
        ...
        candles = []
        for ts, row in data.iloc[::-1].iterrows():
            try:
                candles.append({
                    "datetime": str(ts),
                    "open":     float(row["Open"]),
        ...
        print(f"[get_candles] exception for {ticker}/{interval}: {e}")
        return None
...
def analyze_tf(ticker: str, interval: str) -> dict | None:
    ...
    candles = get_candles(ticker, cfg["interval"], 50)
    ...
    pair     = pair_display(ticker)
    pip_size = 0.01 if "JPY" in pair or "XAG" in pair else 0.0001
...
def scan_market(chat_id, lot_size: float):
    send_message(chat_id,
        ...
        f"Powered by Yahoo Finance — no limits.\n"
    ...
    if cid in pending_signal and settings["lot_size"] is None:
        ...
        if choice in LOT_OPTIONS:
            ...
            try:
                ...
                if not (0.01 <= lot <= 5.0):
                    send_message(chat_id,
                        "❌ Lot size must be between 0.01 and 5.00. "
                        "Try again:"
                    )
                    return True
                ...
                send_message(chat_id,
                    "❌ Invalid lot size. Enter a number e.g: `0.25`")
                return True
        else:
            send_message(chat_id,
                "❌ Please reply with *1*, *2*, *3*, or *4*\n\n"
                "1️⃣ Micro | 2️⃣ Mini | 3️⃣ Standard | 4️⃣ Custom"
            )
            return True
...
def handle(chat_id, text: str):
    ...
    if text == "/start":
        user_settings[str(chat_id)] = {"account": None, "lot_size": None}
        send_message(chat_id,
            ...
            "Powered by Yahoo Finance — no limits.\n"
        ...
        for i, a in enumerate(user_alerts, 1):
            lines.append(
                f"{i}. *{a['pair']}* — {a['direction']} `{a['price']}`")
        send_message(chat_id, "\n".join(lines))
