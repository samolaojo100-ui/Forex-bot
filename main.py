    ...
    try:
        ...
        for i in xs:
            ...
            ax1.bar(i, abs(c - o), bottom=min(o, c), color=color,
                    width=0.7, alpha=0.95, zorder=3)

        # EMA lines
        ax1.axhline(ema50_val,  color="#58a6ff", linewidth=1.5,
                    linestyle="--", alpha=0.9, label=f"EMA50  {round(ema50_val,5)}", zorder=4)
        ax1.axhline(ema200_val, color="#e3b341", linewidth=1.5,
                    linestyle="--", alpha=0.9, label=f"EMA200 {round(ema200_val,5)}", zorder=4)

        # Entry / TP / SL lines
        ax1.axhline(entry, color="#ffffff", linewidth=1.8,
                    linestyle="-", alpha=1.0, label=f"Entry  {entry}", zorder=5)
        ax1.axhline(tp, color="#3fb950", linewidth=1.8,
                    linestyle="-", alpha=1.0, label=f"TP     {tp}", zorder=5)
        ax1.axhline(sl, color="#f85149", linewidth=1.8,
                    linestyle="-", alpha=1.0, label=f"SL     {sl}", zorder=5)

        # Shaded TP/SL zone
        ax1.fill_between(xs, entry, tp,
                         color="#3fb950" if direction=="BUY" else "#f85149",
                         alpha=0.07)
        ax1.fill_between(xs, entry, sl,
                         color="#f85149" if direction=="BUY" else "#3fb950",
                         alpha=0.07)
        ...
        ax1.set_title(f"  {name}  |  {dir_icon}  |  Daily Chart",
                      color=dir_color, fontsize=14, fontweight="bold",
                      loc="left", pad=12)

        # Legend
        leg = ax1.legend(loc="upper left", facecolor="#161b22",
        ...
        for j in range(len(closes)):
            rsi_vals.append(calc_rsi(closes[max(0,j-14):j+1]))
        ...
        ax2.fill_between(xs, rsi_vals, 50,
                         ...
                         color="#bc8cff", alpha=0.12)
        ax2.fill_between(xs, rsi_vals, 50,
                         where=[r < 50 for r in rsi_vals],
                         color="#bc8cff", alpha=0.06)
        ax2.set_ylim(0, 100)
        ...
        # Watermark
        fig.text(0.98, 0.02, "SamSos Forex Bot",
                 color="#30363d", fontsize=9, ha="right", va="bottom")
...
def detect_structure(closes, highs, lows):
    ...
    if hh and hl:   return "bullish","✅ Structure: Higher Highs + Higher Lows — Uptrend"
    elif lh and ll: return "bearish","✅ Structure: Lower Highs + Lower Lows — Downtrend"
    elif hh and ll: return "breakout","⚠️ Structure: Expansion — possible breakout"
    else:           return "ranging","⚠️ Structure: Ranging — no clear trend"
...
def get_market_status():
    ...
    if weekday==6 and hour<21:
        return False,"closed",f"Market closed — reopens in ~{21-hour} hours (Sun 21:00 UTC)"
    ...
    if not sessions: sessions=["Inter-session (low liquidity)"]
    if 8<=hour<12:     quality="BEST — London session (high liquidity)"
    elif 13<=hour<17:  quality="BEST — London/NY overlap (highest liquidity)"
    elif 12<=hour<21:  quality="GOOD — New York session"
    elif 7<=hour<16:   quality="GOOD — London session"
    else:              quality="LOW — Asian session (less movement)"
    return True," + ".join(sessions),quality

def get_news_warnings():
    try:
        r=requests.get("https://api.rss2json.com/v1/api.json",
            params={"rss_url":"https://www.forexlive.com/feed/news"},timeout=10)
...
def get_news():
    try:
        ...
        if not items: return "No news available right now."
        lines=["📰 *Latest Forex News*\n"+"━"*22]
        for item in items: lines.append(f"• {item.get('title','')}")
        lines.append(f"\n⏰ {now()}")
        return "\n".join(lines)
    except: return "Could not fetch news right now."
...
def get_performance():
    ...
    lines=[
        ...
        "━"*22,
        "Excellent!" if wr>=60 else "Keep improving." if wr>=45 else "Review your entries carefully.",
    ]
    return "\n".join(lines)
...
def score_pair(name,ticker):
    ...
    if chg>0.1:       bull+=1; factors.append(f"Strong upward momentum today (+{chg}%)")
    elif chg<-0.1:    bear+=1; factors.append(f"Strong downward momentum today ({chg}%)")
    else:             factors.append(f"Weak momentum today ({chg}%)")
    ...
    else:             factors.append("Price in middle of recent range")
    if ema_gap>0.15:
        if ema50>ema200: bull+=1
        else: bear+=1
        factors.append(f"Strong trend strength ({ema_gap}% EMA gap)")
    ...
    for ctype,cdesc,cscore in candles:
        if "bull" in ctype:   bull+=cscore; factors.append(cdesc.replace("✅ ","").replace("⚠️ ",""))
        elif "bear" in ctype: bear+=cscore; factors.append(cdesc.replace("✅ ","").replace("⚠️ ",""))
        else:                 factors.append(cdesc.replace("✅ ","").replace("⚠️ ",""))
    struct,struct_desc=detect_structure(closes,highs,lows)
    if struct=="bullish":   bull+=1; factors.append(struct_desc.replace("✅ ",""))
    elif struct=="bearish": bear+=1; factors.append(struct_desc.replace("✅ ",""))
    else:                   factors.append(struct_desc.replace("⚠️ ",""))
    is_buy=bull>bear; winning=bull if is_buy else bear; score=winning
    return {
...
def format_signal(d, acc, risk, tf_biases=None, news_warns=None):
    price=d["price"]; direction=d["direction"]; score=d["score"]
    is_gold=d["is_gold"]; is_jpy=d["is_jpy"]; max_score=d["bull"]+d["bear"]
    ...
    factors_txt="\n".join(f"  {i+1}. {f}" for i,f in enumerate(d["factors"][:7]))
    tf_txt=""
    if tf_biases:
        ...
        tf_txt=(f"━━━━━━━━━━━━━━━━━━━━━━\n"
                f"TIMEFRAME CONFIRMATION ({agreements}/{len(tf_biases)} agree)\n"
                +"\n".join(tf_lines)+"\n")
    news_txt=""
    if news_warns:
        news_txt=(f"━━━━━━━━━━━━━━━━━━━━━━\n"
                  f"⚠️ NEWS ALERT — Trade with caution:\n"
...
def find_solid_pairs(cid, acc, risk):
    ...
    if not is_open:
        send(cid,
            ...
            f"━━━━━━━━━━━━━━━━━━━━━━\n"
            f"{status}\n\n"
            f"I will notify you when the market reopens.\n"
            f"━━━━━━━━━━━━━━━━━━━━━━",
            buttons=MENU)
        return
    send(cid,
    ...
    if news_warns:
        send(cid,
            ...
            "━━━━━━━━━━━━━━━━━━━━━━\n"
            +"\n".join(f"• {w}" for w in news_warns[:3])
            +"\n\nConsider waiting for news to pass before trading.")
    results=[]
    ...
    if not solid:
        ...
        send(cid,"No very high confidence setups right now.\nShowing top 3 best available.\nConsider waiting for stronger setups.")
    else:
        send(cid,f"Found *{len(solid)}* solid setup(s) — HIGH confidence only.")
    for d in solid:
        try:
        ...
        if chart_path:
            send_photo(cid,chart_path,caption=f"*{d['name']}* — {d['direction']} Signal")
        send(cid,msg)
...
def best_setup(cid, acc, risk):
    ...
    d=best; color="🟢" if d["direction"]=="BUY" else "🔴"
    chart_path=generate_chart(d["name"],d["ticker"],d["direction"],
                               d["price"],
...
def handle_setup(cid,txt,s):
    ...
    if wait=="waiting_account":
        ...
        except: send(cid,"Please type just the number e.g. *500*")
    elif wait=="waiting_risk":
        try:
            pct=float(txt.replace("%",""))
            if pct>10: send(cid,"Too high. Please enter between 1-5%"); return
            ...
            send(cid,
                ...
                buttons=MENU)
        except: send(cid,"Please type just the number e.g. *2*")

def handle(cid,txt):
    s=ls(); txt=txt.strip(); wait=get_wait(cid)
    ...
    cmd=txt.lower()
    if cmd=="/start":
        if not s.get("setup_done"): start_setup(cid)
        else:
            is_open,session,status=get_market_status()
        ...
        except: send(cid,"Could not calculate strength right now.",buttons=MENU)
        ...
        j=lj()
        if not j: send(cid,"No trades logged yet.",buttons=MENU); return
