...
SCAN_PAIRS = [
    ...
    "NZD/JPY", "CHF/JPY", "EUR/CHF"
]

# ✅ RISK PROFILES — no custom risk, user picks a style
RISK_PROFILES = {
...
def set_bot_commands():
    commands = [
        ...
        {"command": "signal",       "description": "Smart scan all 20 pairs for best signals"},
        {"command": "analyze",      "description": "Analyze one pair e.g analyze EURUSD"},
        {"command": "pairs",        "description": "List all 20 scannable pairs"},
    ...
    try:
        ...
        if r.json().get("result"):
            ...
            print(f"[set_bot_commands] {r.json()}")
    except Exception as e:
...
def calc_lot_size(account: float, risk_pct: float,
    ...
    return max(0.01, min(round(raw, 2), 5.0))  # ✅ Capped at 5.0 lots
...
def scan_market(chat_id, account: float, risk_pct: float):
    """
    ✅ Scans all 20 pairs dynamically.
    Only returns pairs where at least 3 TFs agree + strong indicators.
    Ranks by total score — best pairs first.
    """
    ...
    for pair in SCAN_PAIRS:
        ...
        # ✅ Only include pairs with avg indicator score >= 3/5
        if avg_score >= 3.0:
    ...
    summary_lines = [
        ...
        f"Scanned: {len(SCAN_PAIRS)} pairs | Unavailable: {unavailable}\n"
    ]
    ...
    # ✅ Send full signal for each strong pair
...
def ask_account(chat_id, context: dict):
    """Step 1 — ask for account balance."""
...
def handle_pending(chat_id, text: str) -> bool:
    ...
    # Step 1 — waiting for account balance
    ...
    # Step 2 — waiting for risk profile choice
    if cid in pending_signal and settings["risk_pct"] is None:
        ...
        if pair:
            # Single pair analyze — run in thread
            def run_single():
                results = []
                for tf in TIMEFRAME_CONFIG:
                    r = analyze_tf(pair, tf)
                ...
                total_score = sum(
                    r["buy_score"] if r["direction"] == "BUY"
                    else r["sell_score"] for r in results
                )
                ...
                for r in results:
                    ...
                    tp_p = pips(pair, abs(r["entry"] - r["tp"]))
                    lot  = calc_lot_size(
                        settings["account"], settings["risk_pct"], sl_p, pair
                    )
                    agree_mark   = "✅" if r["label"] in agreeing else "⬜"
                    warning_line = f"\n   {r['warning']}" if r["warning"] else ""
                    indicators   = ", ".join(r["indicators"]) if r["indicators"] else "mixed"
                ...
                msg  = (
                    ...
                    + "\n\n".join(tf_lines) +
                    f"\n\n💬 *Market Context*\n{news}\n\n"
                    f"💰 Account: ${settings['account']:,.0f} | "
                    f"Risk: {settings['risk_pct']}% "
                    f"(${settings['account'] * settings['risk_pct'] / 100:,.0f} per trade)"
                )
                send_message(chat_id, msg)
...
def handle(chat_id, text: str):
    ...
    if text == "/start":
        ...
        send_message(chat_id,
            ...
            "📌 *Commands*\n"
            "• /signal — smart scan all 20 pairs\n"
            "• /analyze EURUSD — single pair deep analysis\n"
            "• /pairs — list all 20 pairs\n"
...
def session_job(session_name: str, flag: str):
    ...
    send_message(CHAT_ID,
        ...
        f"🔍 Scanning 20 pairs dynamically...\n"
