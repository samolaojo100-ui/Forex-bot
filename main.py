...
def send(cid, txt, buttons=None):
    ...
    if buttons:
        ...
        for row in buttons:
            inline_row = []
            for btn in row:
                inline_row.append({"text": btn, "callback_data": btn.lower().strip()})
            inline.append(inline_row)
        body["reply_markup"] = {"inline_keyboard": inline}
    try:
...
def answer_callback(callback_id):
    try:
        requests.post(f"{URL}/answerCallbackQuery", json={"callback_query_id": callback_id}, timeout=5)
    except:
        pass
...
def get_tf_bias(ticker):
    ...
    biases = {}
    for tf_name, period, interval in [("5m","2d","5m"),("15m","5d","15m"),("30m","15d","30m"),("1h","1mo","1h"),("4h","3mo","1h"),("1d","3mo","1d")]:
        try:
...
def score_pair(name, ticker):
    ...
    return {
        ...
        "r_high": r_high, "r_low": r_low, "tf_name": "1d",
...
def format_signal(d, acc, risk, tf_biases=None, tf_agree=False):
    ...
    # Timeframe-specific SL/TP in pips
    tf_name = d.get("tf_name", "signal")
    if is_gold:
        sl_pips = {"5m":20,"15m":30,"30m":50,"1h":80,"4h":150,"1d":250}.get(tf_name, 50)
    elif is_jpy:
        sl_pips = {"5m":8,"15m":12,"30m":20,"1h":30,"4h":60,"1d":100}.get(tf_name, 20)
    else:
        sl_pips = {"5m":8,"15m":12,"30m":18,"1h":25,"4h":45,"1d":80}.get(tf_name, 15)
    tp_pips = sl_pips * 2
            ...
            if ups.get("ok"):
                for u in ups.get("result", []):
                    ...
                    if cb:
                        ...
                        handle(cb["message"]["chat"]["id"], cb["data"])
                    elif "text" in msg:
