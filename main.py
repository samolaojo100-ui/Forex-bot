...
SCAN_PAIRS = [
    ...
    "OANDA:USD_CHF", "OANDA:USD_CAD", "OANDA:AUD_USD",
    "OANDA:NZD_USD",
    # Euro crosses
    "OANDA:EUR_GBP", "OANDA:EUR_JPY", "OANDA:EUR_AUD",
    "OANDA:EUR_CAD", "OANDA:EUR_CHF", "OANDA:EUR_NZD",
    # GBP crosses
    "OANDA:GBP_JPY", "OANDA:GBP_AUD", "OANDA:GBP_CAD",
    "OANDA:GBP_CHF", "OANDA:GBP_NZD",
    # JPY crosses
    "OANDA:AUD_JPY", "OANDA:CAD_JPY", "OANDA:CHF_JPY",
    "OANDA:NZD_JPY",
    # AUD/Other
    "OANDA:AUD_CAD", "OANDA:AUD_CHF", "OANDA:AUD_NZD",
    "OANDA:CAD_CHF", "OANDA:NZD_CAD", "OANDA:NZD_CHF",
    # Metals
...
def pair_display(symbol: str) -> str:
    raw = symbol.replace("OANDA:", "").replace("_", "/")
    return raw

# ✅ LOT SIZE OPTIONS
LOT_OPTIONS = {
    "1": {"label": "🔵 Micro",    "lot": 0.01},
    "2": {"label": "🟢 Mini",     "lot": 0.10},
    "3": {"label": "🟡 Standard", "lot": 0.50},
    "4": {"label": "🔴 Custom",   "lot": None},
}
...
def get_settings(chat_id: str) -> dict:
    ...
    if cid not in user_settings:
        user_settings[cid] = {"account": None, "lot_size": None}
...
def check_price_alerts():
    while True:
        ...
        for alert in price_alerts:
            try:
                symbol  = "OANDA:" + alert["pair"].replace("/", "_")
                candles = get_candles(symbol, "5min", 1)
                if not candles:
                    continue
                current_price = float(candles[0]["close"])
                hit = (
                    (alert["direction"] == "above" and
                     current_price >= alert["price"]) or
                    (alert["direction"] == "below" and
                     current_price <= alert["price"])
                )
                if hit:
                    send_message(alert["chat_id"],
                        f"🔔 *PRICE ALERT TRIGGERED!*\n\n"
                        f"*{alert['pair']}* has reached "
                        f"*{current_price:.5f}*\n"
                        f"Your alert was set at *{alert['price']}*\n\n"
                        f"⚡ Run `/analyze "
                        f"{alert['pair'].replace('/', '')}` now!"
                    )
...
def set_bot_commands():
    commands = [
        {"command": "start",
         "description": "Welcome and all commands"},
        {"command": "signal",
         "description": "Smart scan full forex market"},
        {"command": "analyze",
         "description": "Analyze one pair e.g analyze EURUSD"},
        {"command": "pairs",
         "description": "List all scannable pairs"},
        {"command": "news",
         "description": "AI market context e.g news EURUSD"},
        {"command": "alert",
         "description": "Set price alert e.g alert EURUSD above 1.1200"},
        {"command": "alerts",
         "description": "View your active alerts"},
        {"command": "cancelalerts",
         "description": "Cancel all your alerts"},
        {"command": "settings",
         "description": "View your current settings"},
        {"command": "reset",
         "description": "Reset account and lot size"},
        {"command": "app",
         "description": "Open SOS Trading Mini App"},
    ]
    try:
...
def get_candles(symbol: str, interval: str, size: int = 50):
    """
    ...
    try:
        ...
        from_time = now - seconds[interval]
        ...
        if data.get("s") != "ok":
            print(f"[get_candles] Finnhub error for "
                  f"{symbol}/{interval}: {data.get('s')}")
            return None
        ...
        for i in range(len(times) - 1, -1, -1):
            candles.append({
                "datetime": str(datetime.fromtimestamp(
                    times[i], tz=timezone.utc)),
                "open":     opens[i],
                "high":     highs[i],
                "low":      lows[i],
                "close":    closes[i],
        ...
        print(f"[get_candles] exception for {symbol}/{interval}: {e}")
        return None
...
def calc_bollinger(closes: list,
    ...
    std    = (sum((c - sma) ** 2
                  for c in closes[:period]) / period) ** 0.5
    upper  = round(sma + 2 * std, 5)
    lower  = round(sma - 2 * std, 5)
    middle = round(sma, 5)
    return upper, middle, lower

def calc_stochastic(candles: list,
                    k_period: int = 14) -> tuple[float, float]:
    ...
    if highest_high == lowest_low:
        return 50.0, 50.0
    k        = round(((closes[0] - lowest_low) /
                      (highest_high - lowest_low)) * 100, 2)
    k_values = []
...
# ── SIGNAL LOGIC ──────────────────────────────────────────────────────────────
TIMEFRAME_CONFIG = {
    "5min":  {"interval": "5min",  "atr_sl_mult": 1.5,
              "atr_tp_mult": 2.5, "label": "5M",    "min_sl_pips": 8},
    "15min": {"interval": "15min", "atr_sl_mult": 1.5,
              "atr_tp_mult": 2.5, "label": "15M",   "min_sl_pips": 12},
    "1h":    {"interval": "1h",    "atr_sl_mult": 2.0,
              "atr_tp_mult": 3.0, "label": "1H",    "min_sl_pips": 20},
    "4h":    {"interval": "4h",    "atr_sl_mult": 2.0,
              "atr_tp_mult": 3.0, "label": "4H",    "min_sl_pips": 30},
    "1day":  {"interval": "1day",  "atr_sl_mult": 2.5,
              "atr_tp_mult": 4.0, "label": "Daily", "min_sl_pips": 50},
}

def analyze_tf(symbol: str, interval: str) -> dict | None:
    cfg     = TIMEFRAME_CONFIG[interval]
    candles = get_candles(symbol, cfg["interval"], 50)
    ...
    tp_dist = round(sl_dist * (cfg["atr_tp_mult"] /
                               cfg["atr_sl_mult"]), 5)
    ...
    change = round(
        ((candles[0]["close"] - candles[1]["close"]) /
         candles[1]["close"]) * 100, 4
    ) if len(candles) > 1 else 0.0
...
def pips(pair: str, price_dist: float) -> float:
    ...
    if "XAU" in pair:
        return round(price_dist / 0.1, 1)
    if "XAG" in pair:
        return round(price_dist / 0.001, 1)
...
def get_news_summary(pair: str) -> str:
    prompt = (
        f"You are a senior forex analyst. In 3 plain-English sentences "
        f"explain the current market sentiment for {pair}. "
...
def format_signal_msg(pair: str, results: list, agreeing: list,
                      lot_size: float) -> str:
    direction = next(
        (r["direction"] for r in results if r["label"] in agreeing),
        results[0]["direction"]
    )
...
# ── MARKET SCANNER ────────────────────────────────────────────────────────────
def scan_market(chat_id, lot_size: float):
    send_message(chat_id,
        f"🔍 *Full Market Scanner Running...*\n\n"
        f"Scanning *{len(SCAN_PAIRS)} pairs* across 5 timeframes.\n"
        f"Powered by Finnhub — reliable & fast.\n"
    ...
    for symbol in SCAN_PAIRS:
        ...
        for tf in TIMEFRAME_CONFIG:
            ...
            if r:
                results.append(r)
            time.sleep(0.5)  # ✅ Finnhub free: 60 calls/min
        ...
        total_score = sum(
            r["buy_score"] if r["direction"] == "BUY"
            else r["sell_score"]
            for r in results
        )
    ...
    summary_lines = [
        f"✅ *Scan Complete — {len(strong_signals)} Strong Signal(s)*\n"
    ...
    for s in strong_signals:
        msg = format_signal_msg(
            s["pair"], s["results"], s["agreeing"], lot_size
        )
...
def ask_lot_size(chat_id):
    send_message(chat_id,
        "📦 *Step 2 of 2 — Lot Size*\n\n"
        "Choose your lot size:\n\n"
        "1️⃣ — 🔵 *Micro* (0.01 lots) — Smallest, safest\n"
        "2️⃣ — 🟢 *Mini* (0.10 lots) — Small risk\n"
        "3️⃣ — 🟡 *Standard* (0.50 lots) — Medium risk\n"
        "4️⃣ — 🔴 *Custom* — Type your own e.g `0.25`\n\n"
...
def handle_pending(chat_id, text: str) -> bool:
    ...
    if cid in pending_signal and settings["account"] is None:
        try:
            ...
            if amount < 10:
                send_message(chat_id,
                    "❌ Account must be at least $10. Try again:")
                return True
            settings["account"] = amount
            ask_lot_size(chat_id)
            return True
        except ValueError:
            send_message(chat_id,
                "❌ Please enter a valid number e.g: `20000`")
            return True

    # Step 2 — lot size
    if cid in pending_signal and settings["lot_size"] is None:
        choice = text.strip()

        if choice in LOT_OPTIONS:
            opt = LOT_OPTIONS[choice]
            if opt["lot"] is None:
                send_message(chat_id,
                    "✏️ Enter your custom lot size e.g: `0.25`\n"
                    "Max allowed: *5.00 lots*"
                )
                pending_signal[cid]["awaiting_custom_lot"] = True
                return True
            settings["lot_size"] = opt["lot"]

        elif pending_signal[cid].get("awaiting_custom_lot"):
            try:
                lot = float(choice)
                if not (0.01 <= lot <= 5.0):
                    send_message(chat_id,
                        "❌ Lot size must be between 0.01 "
                        "and 5.00. Try again:"
                    )
                    return True
        ...
        if pair:
            ...
            def run_single():
                ...
                if require_agreement and not direction:
                    buys  = sum(1 for r in results
                                if r["direction"] == "BUY")
                    sells = sum(1 for r in results
                                if r["direction"] == "SELL")
                    detail = "\n".join(
                ...
                msg = format_signal_msg(pair, results, agreeing, lot)
                send_message(chat_id, msg)
...
def handle(chat_id, text: str):
    ...
    if text == "/start":
        user_settings[str(chat_id)] = {
            "account": None, "lot_size": None
        }
        send_message(chat_id,
            "👋 *Welcome to SOS Trading Signal Bot*\n\n"
            "📌 *Commands*\n"
            "• /signal — smart scan full forex market\n"
            ...
            "• /reset — reset account & lot size\n"
            "• /app — open Mini App\n\n"
            "🔍 *Smart Scanner*\n"
            "Scans 30 pairs — full market.\n"
            "Powered by Finnhub — no blocks.\n"
            "Minimum 3 TFs must agree.\n"
            "Pairs ranked by strength score.\n\n"
            "📊 *Timeframes*: 5M | 15M | 1H | 4H | Daily\n"
            "🔒 *Indicators*: EMA + MACD + RSI + BB + Stoch\n"
            "📦 *Lot Sizes*: Micro | Mini | Standard | Custom\n\n"
        ...
        send_message(chat_id,
            "🔄 Settings reset.\n"
            "Account and lot size will be asked fresh next signal."
        )
        ...
        keyboard = {
            "inline_keyboard": [[{
                ...
                "web_app": {
                    "url": "https://sparkly-biscochitos-5477ba.netlify.app/"
                }
            }]]
        }
        requests.post(f"{BASE_URL}/sendMessage", json={
        ...
        ask_account(chat_id, {
            "pair": pair, "require_agreement": False
        })

    elif text == "/settings":
        s = get_settings(chat_id)
        if s["account"] is None or s["lot_size"] is None:
            send_message(chat_id,
        ...
        user_alerts = [a for a in price_alerts
                       if str(a["chat_id"]) == str(chat_id)]
        if not user_alerts:
            send_message(chat_id,
                "📭 No active alerts.\n"
        ...
        for i, a in enumerate(user_alerts, 1):
            lines.append(
                f"{i}. *{a['pair']}* — "
                f"{a['direction']} `{a['price']}`"
            )
        send_message(chat_id, "\n".join(lines))

    elif text == "/cancelalerts":
        before = len([a for a in price_alerts
                      if str(a["chat_id"]) == str(chat_id)])
        ...
        pair = (raw[:3] + "/" + raw[3:]) if "/" not in raw else raw
        send_message(chat_id,
            f"📰 Fetching AI analysis for *{pair}*...")
...
def handle_web_app(chat_id, raw_data: str):
    try:
        ...
        if data["action"] == "signal":
            ...
            settings["lot_size"] = float(data.get("lot_size", 0.10))
            send_message(chat_id,
                f"✅ Settings updated!\n"
                f"💰 Account: *${settings['account']:,.2f}*\n"
                f"📦 Lot Size: *{settings['lot_size']} lots*"
            )
    except Exception as e:
...
def session_job(session_name: str, flag: str):
    ...
    s        = get_settings(CHAT_ID)
    lot_size = s["lot_size"] or 0.10
    send_message(CHAT_ID,
        f"{flag} *{session_name} Session — Auto Scan*\n\n"
        f"🔍 Scanning full forex market...\n"
        f"Minimum 3 TFs must agree.\n"
        f"📦 Using lot size: *{lot_size}*"
    )
    scan_market(CHAT_ID, lot_size)
