...
def format_signal(sig, index: int = 1, total: int = 1) -> str:
    ...
    # Verdict emoji
    verdict_emoji = {"FIRE": "🔥", "CAUTION": "⚠️", "NO TRADE": "🚫"}.get(
        getattr(agents, "verdict", "CAUTION"), "⚠️"
    )
    ...
    msg = (
        ...
        f"{verdict_emoji} Verdict: *{getattr(agents, 'verdict', 'CAUTION')}*"
        f" — _{getattr(agents, 'verdict_reason', '')}_\n"
        f"{candle_line}\n"
        f"{warn_block}\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━\n"
        ...
        f"\n"
        f"📰 *News Sentiment:*\n"
        f"_{getattr(agents, 'news_sentiment', 'N/A')}_\n"
        f"\n"
        f"🌍 *Fundamental Agent:*\n"
        f"_{getattr(agents, 'fundamental', 'N/A')}_\n"
        f"\n"
        f"🎯 *Coordinator:*\n"
        f"_{getattr(agents, 'coordinator', 'N/A')}_\n"
        f"\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━\n"
...
def format_scanning(crypto_only: bool = False) -> str:
    pairs = "3 crypto pairs" if crypto_only else "4 forex + 3 crypto pairs"
    return (
        f"🔄 *Scanning {pairs}…*\n\n"
