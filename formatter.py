...
def format_no_signal() -> str:
    return (
        ...
        "📌 _All 3 timeframes (15m/1h/4h) must agree._\n"
        "⏳ _Auto-scan retries in 30 min._"
    )


def format_scanning(crypto_only: bool = False) -> str:
    scope = "12 crypto pairs (BTC, ETH, SOL…)" if crypto_only else "47 forex + 12 crypto pairs"
    return (
        f"🔄 *Scanning the market…*\n\n"
        f"Checking {scope}\n"
        f"across `15min` / `1h` / `4h`\n\n"
...
def format_status(session_name: str, is_active: bool, next_scan_min: int, balance_text: str) -> str:
    ...
    return (
        ...
        f"⏱ Next auto-scan: *{next_scan_min} min*\n"
        f"💰 Balance: *{balance_text}*\n\n"
        f"₿ Crypto scans 24/7\n"
        f"💱 Forex scans weekdays only"
    )
