...
def format_signal(sig, index: int = 1, total: int = 1) -> str:
    """Format a valid trade signal for Telegram."""
    ...
    # S/R warning line — only shown if present
    ...
    return (
        ...
        f"Pair Score: *{sig.score}/25* | {a_label}{sr_line}\n"
...
def format_no_trade(result: dict) -> str:
    """
    Format a NO TRADE result — shown when signal_engine blocked a pair.
    Only shown on manual /signal, never on auto-scan.
    """
...
def format_no_signal() -> str:
    """Shown when scan finds zero qualifying signals at all."""
    return (
        "🔍 *Scan Complete*\n\n"
...
def format_status(session: str, active: bool, mins: int, bal: str,
    ...
    """
    Status message — now optionally shows upcoming high-impact events.
    upcoming_events: list of dicts from news_filter.get_upcoming_events()
    """
    ...
    if upcoming_events:
        lines = []
        for e in upcoming_events[:5]:  # max 5
